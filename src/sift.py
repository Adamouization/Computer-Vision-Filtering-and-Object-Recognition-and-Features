...
def get_features(img, thr):
    """
    Calculates SIFT key points.
    :param img: the original image
    :param thr: the threshold
    :return: the list of SIFT keypoints
    """
    ...
    # assign different kernel sizes for different image scales and different levels.
    # More specifically, start from sigma and k and increase k's exponent by 1 for progression within a level
    # and increase k's exponent by 2 between scales
        ...
        for i in range(1, 4):
            for j in range(1, img_at_scale[n].shape[0] - 1):
                for k in range(1, img_at_scale[n].shape[1] - 1):
                    ...
                    # below, elements of each pyramids that are larger or smaller than its 26 immediate neighbors
                    # in space and scale are labeled as extrema
                    ...
                    # the below code will filter them further by using a threshold based on the eigenvalues of
                    # the Hessian matrix
                    if max or min:
                        ...
                        # inequality used for separation of interest points referring to edges in contrast
                        # to the ones referring to corners
                        thr_ratio = (tr_d ** 2) / det_d
                        r_ratio = ((r + 1) ** 2) / r

                        if thr_ratio > r_ratio:
                            # mark the matrix with a value to know which are the coordinates of the interest points
            ...
            for j in range(1, img_at_scale[n].shape[0] - 1):
                for k in range(1, img_at_scale[n].shape[1] - 1):
                    ...
                    if max_min_at_scale[n][j, k, i] == 1:
                        ...
                        # this is the reason earlier we converted radians to this different type of degree
                        buckets = np.zeros([36, 1])

                        # for each bin, compute weight inside it,
                        # keeping in mind that the magnitudes close to the keypoint of interest are more important
                        for x in range(int(-1 * win_radius * 2), int(win_radius * 2) + 1):
                            ...
                            for y in range(-1 * square_to_circle_shape, square_to_circle_shape + 1):
                                ...
                                # then maximum weight is reached, for borders, weight is smaller
...
def draw_sift_boxes_and_directions(img, keypoints):
    """
    Draws a box and an arrow to represent SIFT keypoints
    :param img: the image to draw on
    :param keypoints: the list of SIFT keypoints
    :return: None
    """
    testing_img = img
    for i in range(0, keypoints.shape[0]):
